...
ax.plot(fprsA, tprsA, label="{:s}  {:5.3f}".format("QDA", auc));
ax.plot([0,1],[0,1]);
ax.legend()
...
for train_i, test_i in folder.split(train_data, train_lbl_data):
    best_f1_score, n_cmp_neg, n_cmp_pos = evaluate(10,8,train_data.iloc[train_i], train_lbl_data.iloc[train_i], train_data.iloc[test_i], train_lbl_data.iloc[test_i])
    f1+=best_f1_score
